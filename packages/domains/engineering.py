...
FALLBACK_TOPICS = ("核心概念", "公理", "基本定理", "重要结论")
...
def build_study_queries(source_text: str = "", outline: str = "") -> list[StudyQuery]:
    ...
    return [
        StudyQuery(
            "definitions",
            "定义与基本概念",
            f"{topic_text} 定义 概念 符号 变量 物理量 术语",
            required_any,
        ),
        StudyQuery(
            "theorems",
            "定理与公式",
            f"{topic_text} 定理 公式 定律 推论 方程 关系式",
            required_any,
        ),
        StudyQuery(
            "derivations",
            "推导与证明",
            f"{topic_text} 推导 证明 证明思路 关键步骤 推导过程",
            required_any,
        ),
        StudyQuery(
            "algorithms",
            "算法与流程",
            f"{topic_text} 算法 流程 步骤 协议 方法 设计 实现",
            required_any,
        ),
        StudyQuery(
            "applications",
            "应用与示例",
            f"{topic_text} 应用 示例 例题 典型问题 实现 工程应用",
            required_any,
        ),
        StudyQuery(
            "pitfalls",
            "注意事项",
            f"{topic_text} 注意事项 易错点 边界条件 特殊情况 陷阱",
            required_any,
        ),
    ]
...
def build_generation_prompt(
    ...
    system = "你是面向工科学生的课件整理助手。严格遵守用户提供的 soul。重视公式、推导、流程和工程应用。正文应像高质量学习资料，不要展示工程调试痕迹。"
...
3. LaTeX 公式使用 $ 或 $$ 格式完整渲染。
4. 关键知识点附近保留隐藏证据注释，例如 `<!-- evidence: E001 E002 -->`。
5. 缩写第一次出现必须写全称。
6. 不要写"根据证据片段""本 MVP"等工程化表达。
"""
