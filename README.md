...
`JSTUDY_JOBS_DIR` is a fixed topology setting. Admin-managed values such as provider credentials, model selection, Soul/content paths, parser settings, `JSTUDY_MAX_PDF_BYTES`, and retention are loaded from shared `data/settings` when the corresponding process environment variable is empty. A nonempty environment value is an explicit deployment override and cannot be hot-updated through `/admin/settings`.
