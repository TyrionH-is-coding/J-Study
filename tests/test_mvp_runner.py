...
from mvp_runner import (  # noqa: E402
    Chunk,
    DeepTutorRagAdapter,
    RagConfig,
    StudyQuery,
...
"""
    ...
    @patch("mvp_runner.urllib.request.urlopen")
    ...
    @patch("mvp_runner.extract_pdf_pages")
    @patch("mvp_runner.embed_texts")
    def test_run_mvp_writes_links_quality_cache_and_uses_outline(self, embed_texts, extract_pdf_pages):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch(
                "mvp_runner.build_study_queries",
                return_value=[StudyQuery("sample", "Sample", "alpha overview")],
            ), patch("mvp_runner.generate_markdown") as generate_markdown:
