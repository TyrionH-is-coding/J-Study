...
- 为 `material-package.v2` 建立 strict Pydantic contract，所有公开模型禁止未知字段；首期只允许受控 heading、paragraph、list、table、callout、formula blocks 和六类 inline runs。
- 落实 section/block 稳定 identity、heading/callout 枚举、文本长度、block 数量和 table 行列上限；只有 `failed` section 可以没有 blocks。
- 共享 validator 校验 package/section source、section evidence 和 citation run 引用；稳定错误码包括 `unknown_source_id`、`unknown_evidence_id`、`undeclared_section_evidence`。
- quality 不再解析 Markdown，而是直接遍历 typed blocks/citation runs，计算 section、evidence、referenced evidence 和 citation coverage 指标。
...
- Worker 保留小型 legacy v1 分支；v2 wrong package/service identity、unknown source/evidence、duplicate identity 或 malformed schema 均以 `invalid_job_output` 失败，不提交 artifacts/sections。
- `GET /api/jobs/{job_id}/package` 保持 owner check 与 private cache，返回 validated v2 或 bounded legacy v1。v2 read path 会重新校验 persisted package 与 Job-owned source/evidence，OpenAPI 发布 Package、Section、各 block 与 Citation schemas。
...
- `python -m unittest discover -s tests -v`：`318/318` 通过。
- `npm run lint`：通过。
...
- legacy v1 仅提供迁移期 bounded read；新 Job 一律生成 v2，legacy 删除仍需单独审查。
