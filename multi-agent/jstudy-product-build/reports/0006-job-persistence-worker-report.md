...
- `.env.example`：管理员管理字段改为空值回落，public pilot retention 保留 `72`。
...
- `deploy/docker-compose/api.compose.yml`：为 API/Worker 恢复显式 `.env` pass-through，模型无非空默认值。
- `docs/architecture/overview.md`：更新 Job persistence、Worker 与运行设置优先级架构。
- `docs/deployment/server-runbook.md`：统一非空环境 override、空值回落和 public pilot retention 操作合同。
...
- `tests/test_deployment_files.py`：验证 Dockerfile 不覆盖管理员设置、三服务 Compose contract 和可执行 config probe。
...
- `tests/test_job_worker.py`：覆盖双 service mode、settings snapshot、heartbeat、重试、陈旧 Worker、section/artifact 原子完成、六类公开 artifact、悬空 Markdown、package validation、cache 隔离和 retention。
...
- `tests/test_settings.py`：覆盖 exact pilot defaults、非空环境 override 和空环境回落管理员设置。
- `tests/test_web_mvp.py`：覆盖 queued submission、API/Worker 分离、管理端设置热更新、真实 admission+worker sections、六类公开 artifact、重启持久化、状态和原有 route contract。
...
- `python -m unittest discover -s tests -v`：`287/287` 通过。
...
- Compose config probe：向配置注入 probe provider key、chat/embed model、
  `JSTUDY_MAX_PDF_BYTES=123456`、`JSTUDY_JOB_RETENTION_HOURS=72`，解析后的
  API 与 Worker 环境值全部一致。
- `git diff --check`：通过。
